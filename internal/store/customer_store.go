...
func (s *Store) InsertCustomerMessage(
	ctx context.Context,
	userID string,
	userPhone string,
	adminID *string,
...
func (s *Store) GetCustomerHistory(
	ctx context.Context,
	userID string,
	...
	if cursorTime.IsZero() {
		...
		query := `
			SELECT
				...
				sended_by,
				CASE
					WHEN sended_by = 'ADMIN'
						THEN 'Support Admin'
					ELSE COALESCE(full_name, 'Customer')
				END AS sender_name,
				content,
				seen,
				COALESCE(voice_messages, ''),
...
// MarkCustomerMessagesAsSeen updates the seen status.
func (s *Store) MarkCustomerMessagesAsSeen(
	ctx context.Context,
	targetUserID string,
	viewerType string,
) error {
...
func (s *Store) EditCustomerMessage(
	ctx context.Context,
	messageID string,
	content string,
) (*OutgoingMessage, error) {
