...
func (s *Store) InsertDriverMessage(ctx context.Context, userID string, adminID *string, sendedBy string, content string, voice string, photo string, file string) (*Message, error) {
	query := `
		INSERT INTO driver_messages (user_id, admin_id, sended_by, content, voice_messages, photo, file)
		VALUES ($1, $2, $3, $4, $5, $6, $7)
		RETURNING id, user_id, admin_id, sended_by, content, seen, COALESCE(voice_messages, ''), COALESCE(photo, ''), COALESCE(file, ''), created_at, updated_at`

	msg := &Message{}
	err := s.db.QueryRow(ctx, query, userID, adminID, sendedBy, content, voice, photo, file).
		Scan(&msg.ID, &msg.UserID, &msg.AdminID, &msg.SendedBy, &msg.Content, &msg.Seen, &msg.VoiceMessages, &msg.Photo, &msg.File, &msg.CreatedAt, &msg.UpdatedAt)
	if err != nil {
		return nil, err
	}
	return msg, nil
...
func (s *Store) GetDriverHistory(ctx context.Context, userID string, cursorTime time.Time, limit int) ([]OutgoingMessage, error) {
	...
	if cursorTime.IsZero() {
		query := `
			SELECT id, user_id, admin_id, sended_by, 
			       ...
			       content, seen, COALESCE(voice_messages, ''), COALESCE(photo, ''), COALESCE(file, ''), created_at
	...
	for rows.Next() {
		...
		err := rows.Scan(&m.ID, &m.UserID, &m.AdminID, &m.SendedBy, &m.SenderName, &m.Content, &m.Seen, &m.VoiceMessages, &m.Photo, &m.File, &m.CreatedAt)
		if err != nil {
			return nil, err
		}
...
func (s *Store) EditDriverMessage(ctx context.Context, messageID string, content string) (*OutgoingMessage, error) {
	...
	err := s.db.QueryRow(ctx, query, content, messageID).
		Scan(&m.ID, &m.UserID, &m.AdminID, &m.SendedBy, &m.SenderName, &m.Content, &m.Seen, &m.VoiceMessages, &m.Photo, &m.File, &m.CreatedAt)
	if err != nil {
		return nil, err
	}
	m.Type = "EDIT_MESSAGE"
